...
function hasReferenceToResource(requestorId, sourceResource, apiUrl, fhirVersion) {
    const hostname = (0, lodash_1.get)(requestorId, 'hostname', '');
    const resourceType = (0, lodash_1.get)(requestorId, 'resourceType', 'Patient');
    const id = (0, lodash_1.get)(requestorId, 'id', '');
...
function getJwksClient(jwksUri, headers) {
    console.log(`these are the jwks parameters, ${JSON.stringify({
        cache: true,
        cacheMaxEntries: 5,
        cacheMaxAge: 600000,
        rateLimit: true,
